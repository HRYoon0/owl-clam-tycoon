...
| **대상** | User Type **외부**, 게시 상태 **게시됨(프로덕션)** — 아래 주의 참고 |
| **데이터 액세스** | 아래 범위 3개 추가 |
| **클라이언트** | 아래 1-3 참고 |
...
> ⚠️ **게시 상태를 "테스트"로 두면 리프레시 토큰이 7일 뒤 만료됩니다.** 구글이 정한 제한이라
> 서버를 두고 만든 "로그인 계속 유지"가 일주일마다 풀립니다. **프로덕션으로 게시**하면 사라집니다.
>
> 게시해도 "확인되지 않은 앱" 경고는 그대로이고(검증 심사를 받아야 없어짐), 사용자 100명 한도가
> 생깁니다. 개인·소수 사용이면 둘 다 문제되지 않습니다.
>
> **게시한 뒤에는 한 번 연결을 끊었다 다시 로그인하세요.** 리프레시 토큰의 수명은 발급 시점의
> 게시 상태로 정해져서, 테스트 때 받은 토큰은 게시 후에도 7일 뒤 만료됩니다.
>
> 테스트 상태로 남겨둘 경우에는 **프로젝트 소유자라도 테스트 사용자 목록에 따로 등록**해야 합니다.
> 빠뜨리면 로그인 시 `403 access_denied` 가 납니다.
...
| `403 access_denied` | 테스트 상태인데 그 계정이 테스트 사용자에 없음 |
| 일주일쯤 뒤 로그인이 풀림 | 게시 상태가 "테스트"라 리프레시 토큰이 7일 만료. 게시 후 재로그인 |
